document.addEventListener('DOMContentLoaded', () => {
    ...
    class Procedimento {
        ...
        valido() {
            const campos = ['name', 'cor', 'numeroDente', 'faceDente']
            if (this.name === null || this.name === undefined || this.name === '') return false
        ...
        salvar() {
            if (this.valido()) {
                const procedimento = procedimentos.find(prc => prc.name === this.name && prc.numeroDente === this.numeroDente && prc.faceDente === this.faceDente)
        ...
        procedimentos.filter(prc => prc.numeroDente === procedimento.numeroDente && prc.faceDente === procedimento.faceDente).forEach(item => {
            const tr = `
                <tr>
                    ...
                    <td>
                        <a onclick="apagar('${item.name}', ${item.numeroDente}, ${item.faceDente})" class="btn btn-danger">
    ...
    window.apagar = (name, numeroDente, faceDente) => {
        const procd = procedimentos.find(prc => prc.name === name && prc.numeroDente === numeroDente && prc.faceDente === faceDente)
    ...
    const iniciaOdontograma = () => {
        const options = itensProcedimento.map(problema => {
            return `\n<option value='${problema.name}'>${problema.nome}</option>`
        })
